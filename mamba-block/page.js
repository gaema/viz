...
function box(ctx, x, y, w, h, label, color) {
  ctx.save(); ctx.fillStyle = T.n1; ctx.fillRect(x, y, w, h); ctx.strokeStyle = color; ctx.lineWidth = 1.6; ctx.strokeRect(x, y, w, h);
...
mount({
  ...
  draw: (page) => {
    ...
    const dh = (data, rows, dom, leftX, topY, dimFuture) => {
      ...
      if (dimFuture && tcur < L - 1) { ctx.save(); ctx.fillStyle = alphaOf('n0', 0.66); ctx.fillRect(leftX + (tcur + 1) * cell, topY, (L - 1 - tcur) * cell, rows * cell); ctx.restore(); }
      ctx.save(); ctx.strokeStyle = alphaOf(T.accent, 0.8); ctx.lineWidth = 1.6; ctx.strokeRect(leftX + tcur * cell, topY, cell, rows * cell); ctx.restore();   // current position column
      return rect;
    };

    // x (input, left, on the residual spine)
    const x0 = 20, xY = midY - (D * cell) / 2;
    r.label('x [D×L]', x0, xY - 8, { color: T.n14, font: '11px ui-monospace, monospace' });
    xR = dh(cur.x.data, D, xdom, x0, xY, false);
    r.label('drag ↕', x0, xY + D * cell + 12, { color: T.ok, font: '9px ui-monospace, monospace' });

    // in_proj (split)
    const ipx = x0 + hw + 14, ipw = 52;
    box(ctx, ipx, midY - 22, ipw, 44, 'RMSNorm\nin_proj\n(split)', T.accent);
    arrow(ctx, x0 + hw, midY, ipx, midY, T.n9);

    // main lane: conv -> SiLU -> SSM -> y
    const opx = ipx + ipw + 14, opw = 96;
    box(ctx, opx, yTop + (E * cell) / 2 - 17, opw, 34, 'Conv1d→SiLU\n→ SSM (scan)', T.violet);
    arrow(ctx, ipx + ipw, midY - 6, opx + opw / 2, yTop + (E * cell) / 2 + 17, T.violet);
    const yX = opx + opw + 16;
    r.label('y = SSM(u) [E×L]', yX, yTop - 8, { color: T.violet, font: '10px ui-monospace, monospace' });
    yR = dh(Bk.y, E, edom, yX, yTop, true);

    // gate lane: SiLU -> g
    box(ctx, opx, yBot + (E * cell) / 2 - 13, opw, 26, 'SiLU (gate)', T.warn);
    arrow(ctx, ipx + ipw, midY + 6, opx + opw / 2, yBot + (E * cell) / 2 - 13, T.warn);
    r.label('g = SiLU(gate)', yX, yBot - 8, { color: T.warn, font: '10px ui-monospace, monospace' });
    gR = dh(Bk.g, E, edom, yX, yBot, true);

    // gate merge ⊙ -> gated
    const mx = yX + hw + 26;
    ctx.save(); ctx.strokeStyle = T.n14; ctx.fillStyle = T.n0; ctx.lineWidth = 1.8; ctx.beginPath(); ctx.arc(mx, midY, 12, 0, 7); ctx.fill(); ctx.stroke(); ctx.fillStyle = T.n14; ctx.font = '13px ui-monospace, monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('⊙', mx, midY + 1); ctx.restore();
    arrow(ctx, yX + hw, yTop + (E * cell) / 2, mx - 12, midY - 8, T.violet);
    arrow(ctx, yX + hw, yBot + (E * cell) / 2, mx - 12, midY + 8, T.warn);
    const gX = mx + 18;
    r.label('y⊙g [E×L]', gX, midY - (E * cell) / 2 - 8, { color: T.tealDeep, font: '10px ui-monospace, monospace' });
    ...
    arrow(ctx, gX + hw, midY, ox, midY, T.tealDeep);
    const outX = ox + oboxw + 28;
    // residual ⊕
    ctx.save(); ctx.strokeStyle = T.ok; ctx.fillStyle = T.n0; ctx.lineWidth = 1.8; ctx.beginPath(); ctx.arc(outX - 16, midY, 11, 0, 7); ctx.fill(); ctx.stroke(); ctx.beginPath(); ctx.moveTo(outX - 22, midY); ctx.lineTo(outX - 10, midY); ctx.moveTo(outX - 16, midY - 6); ctx.lineTo(outX - 16, midY + 6); ctx.stroke(); ctx.restore();
    arrow(ctx, ox + oboxw, midY, outX - 27, midY, T.tealDeep);
    // residual arc from x down/around to ⊕
    ctx.save(); ctx.strokeStyle = T.ok; ctx.lineWidth = 1.4; ctx.setLineDash([4, 3]); const ry = page.H - 26; ctx.beginPath(); ctx.moveTo(x0 + hw / 2, xY + D * cell); ctx.lineTo(x0 + hw / 2, ry); ctx.lineTo(outX - 16, ry); ctx.lineTo(outX - 16, midY + 11); ctx.stroke(); ctx.setLineDash([]); r.label('residual', x0 + hw / 2 + 6, ry - 6, { color: T.ok, font: '9px ui-monospace, monospace' }); ctx.restore();
    const oY = midY - (D * cell) / 2;
    r.label('out [D×L]', outX, oY - 8, { color: T.n14, font: '11px ui-monospace, monospace' });
