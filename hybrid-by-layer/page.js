...
mount({
  ...
  draw: (page) => {
    ...
    r.label('layer stack (input ↓ bottom → output ↑ top)', pad, topY - 12, { color: T.n14, font: '11px ui-monospace, monospace' });

    let hoverLayer = -1;
    for (let i = 0; i < N; i++) {
      const attn = types[i], y = topY + (N - 1 - i) * rowH, col = attn ? T.warn : T.accent;
      ...
      if (hot) hoverLayer = i;
      ctx.save();
      if (i === fp) { ctx.fillStyle = alphaOf(T.ok, 0.10); ctx.fillRect(pad - 2, rect.y, stackW + 4, rect.h); }   // pulse
      if (hot) { ctx.strokeStyle = T.n14; ctx.lineWidth = 1.5; ctx.strokeRect(pad - 2, rect.y, stackW + 4, rect.h); }
      ctx.fillStyle = T.n11; ctx.font = '10px ui-monospace, monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
      ctx.fillText(`L${i}`, pad, y + rowH / 2);
      // type chip
      ctx.fillStyle = attn ? alphaOf(T.warn, 0.18) : alphaOf(T.accent, 0.16); ctx.fillRect(pad + labelW, y + 2, chipW, rowH - 4);
      ...
      if (attn) {                                                   // KV cache cells (the stored positions)
        ...
        for (let c = 0; c < nc; c++) { ctx.fillStyle = alphaOf(T.warn, 0.35 + 0.4 * (c / nc)); ctx.fillRect(seqX + c * cw, y + 3, cw - 1, rowH - 6); }
        ctx.strokeStyle = alphaOf(T.warn, 0.5); ctx.lineWidth = 1; ctx.strokeRect(seqX, y + 3, nc * cw, rowH - 6);
      } else {                                                      // recurrent state line + a flowing dot
        ctx.strokeStyle = alphaOf(T.accent, 0.55); ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(seqX, cy); ctx.lineTo(seqX + seqW, cy); ctx.stroke();
        const dx = seqX + ((page.t || 0) * 60 + i * 7) % seqW; ctx.fillStyle = T.accent; ctx.beginPath(); ctx.arc(dx, cy, 2.6, 0, 7); ctx.fill();
      }
      // KV badge
      ctx.textAlign = 'left'; ctx.fillStyle = attn ? T.warn : T.n9; ctx.font = '9px ui-monospace, monospace';
    ...
    const line = (k, v, col) => { r.label(k, px, ly, { color: T.n11, font: '11px ui-monospace, monospace' }); if (v != null) r.label(v, px + 6, ly + 13, { color: col || T.n14, font: '12px ui-monospace, monospace' }); ly += v != null ? 32 : 16; };
    r.label('KV cache budget', px, py - 14, { color: T.n14, font: '12px ui-monospace, monospace' });
    line(`pattern: 1 attn / ${P} layers`, `${nSSM} SSM  +  ${nAttn} ATTN  =  ${N}`, T.n14);
    line('KV cache total = #attn·L·2·d', `${fmt(kvTot)} elems`, T.warn);
    line(`vs full attention (${N} layers)`, `${fmt(kvFull)}  →  ${saving}% less KV`, T.ok);
    line('compute', `${nSSM}×O(L) + ${nAttn}×O(L²)`, T.n14);
    ly += 6;
    const focus = hoverLayer >= 0 ? hoverLayer : fp, fAttn = types[focus];
    r.label(`layer L${focus}${hoverLayer >= 0 ? ' (hover)' : ' (forward pass)'}`, px, ly, { color: fAttn ? T.warn : T.accent, font: '11px ui-monospace, monospace' }); ly += 16;
    r.label(fAttn ? 'full attention · O(L²) · holds a KV' : 'SSM · O(L) · recurrent state, no KV', px, ly, { color: T.n12, font: '10px ui-monospace, monospace' }); ly += 14;
    r.label(fAttn ? `cache = [L=${L} × 2·d=${2 * d}] = ${fmt(kvPer)} elems` : `state ≈ [d×d] constant (independent of L)`, px, ly, { color: T.n12, font: '10px ui-monospace, monospace' }); ly += 14;
    r.label('click a layer to flip SSM↔ATTN', px, ly, { color: T.n9, font: '9px ui-monospace, monospace' });
