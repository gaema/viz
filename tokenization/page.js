...
mount({
  ...
  blurb: 'Before a model sees text it must become integer token IDs, and the dominant scheme is Byte-Pair Encoding (BPE). It starts from raw characters and repeatedly does one thing: find the most frequent ADJACENT pair of symbols across the whole corpus (weighted by how often each word appears), merge that pair into a single new symbol, and add a merge rule. Frequent letter sequences ("e"+"s"→"es", "es"+"t"→"est") thus become single tokens, the vocabulary grows, and the number of tokens needed to write the corpus shrinks. That is the BPE training loop GPT/LLaMA tokenizers run — though over BYTES rather than characters (GPT-2/3/4 and Llama-3 use byte-level BPE after a regex pre-split; Llama-1/2 use SentencePiece with a ▁ word-start marker), so the merge rule here is theirs while the starting alphabet is simplified. At inference the learned merge rules are simply replayed on new text. Step (or play) to watch the merges form; the pair table on the right shows the candidates with the winner highlighted; drag a word\'s frequency ↕ to change which pairs win; the merge rules and the vocab/token counts update live. The trailing "_" marks a word boundary.',
  prefer: 'canvas2d',
  aspect: '3 / 2',
  controls: (c, page) => {
