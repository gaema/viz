...
mount({
  ...
  blurb: 'How does an image get into a text LLM? Not through a separate input — it is turned into TOKENS that live in the same embedding space as words. An encoder (a ViT for images, an audio encoder for sound) turns the input into feature vectors; a PROJECTOR (a small MLP, encoder-dim → the LLM’s embedding dim D) maps them into the LLM’s token space; and those "soft" media tokens are SPLICED into the text token sequence at a <image>/<audio> placeholder. The merged sequence — text embeddings and image/audio tokens, every column the same D-dim vector — is then run through ordinary self-attention, which attends across text and media uniformly (the model never "knows" which columns came from pixels). This is exactly how LLaVA, Qwen2-VL, and audio-LMs work. Drag the orange/green media block to move where it is injected; switch the modality (image / audio / both) and the token count; hover a column to see its source.',
  prefer: 'canvas2d',
  aspect: '3 / 2',
  animate: true,
  controls: (c, page) => {
